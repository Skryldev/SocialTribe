...
class PreferentialAttachment {
    ...
    public function preferentialAttachmentScore(int $u, int $v): float {
        if (!isset($this->adjacency[$u]) || !isset($this->adjacency[$v])) {
            return 0.0;
        }
        return count($this->adjacency[$u]) * count($this->adjacency[$v]);
    }
    ...
    public function scoreAllPairs(): array {
        ...
        for ($i = 0; $i < count($nodeList); $i++) {
            for ($j = $i + 1; $j < count($nodeList); $j++) {
                $u = $nodeList[$i];
                $v = $nodeList[$j];
                $score = $this->preferentialAttachmentScore($u, $v);
                if ($score > 0) {
                    $scores[] = [
                        'pair' => [$u, $v],
                        'score' => $score
