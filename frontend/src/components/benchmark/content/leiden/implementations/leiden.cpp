...
private:
    ...
    bool refine_partition() {
        ...
        for (int node : node_list) {
            int current_community = communities[node];
            int best_community = current_community;
            double best_gain = 0.0;
            
            unordered_set<int> communities_seen;
            for (int neighbor : adjacency[node]) {
                int community = communities[neighbor];
                if (communities_seen.find(community) != communities_seen.end()) continue;
...
public:
    ...
    unordered_map<int, int> detect_communities() {
        ...
        while (true) {
            ...
            while (refine_partition()) {
                improved = true;
            }
            
            if (!improved) break;
            
            while (fast_local_move()) {
                improved = true;
            }
