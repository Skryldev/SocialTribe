...
public:
    ...
    double jaccard_similarity(int u, int v) {
        if (adjacency.find(u) == adjacency.end() || adjacency.find(v) == adjacency.end()) {
            return 0.0;
        }

        const auto& neighbors_u = adjacency[u];
        const auto& neighbors_v = adjacency[v];

        int intersection = 0;
        for (int node : neighbors_u) {
            if (neighbors_v.find(node) != neighbors_v.end()) {
                intersection++;
            }
        }

        int union_size = neighbors_u.size() + neighbors_v.size() - intersection;
        if (union_size == 0) {
            return 0.0;
        }
    ...
    vector<pair<pair<int, int>, double>> score_all_pairs() {
        ...
        for (size_t i = 0; i < node_list.size(); i++) {
            for (size_t j = i + 1; j < node_list.size(); j++) {
                int u = node_list[i];
                int v = node_list[j];
                double score = jaccard_similarity(u, v);
                if (score > 0) {
