...
impl JaccardSimilarity {
    ...
    pub fn jaccard_similarity(&self, u: i32, v: i32) -> f64 {
        if let (Some(neighbors_u), Some(neighbors_v)) = (self.adjacency.get(&u), self.adjacency.get(&v)) {
            ...
            let union_size = neighbors_u.len() + neighbors_v.len() - intersection;
            
            if union_size == 0 {
                return 0.0;
            }
            
            return intersection as f64 / union_size as f64;
        }
        0.0
    }
    ...
    pub fn score_all_pairs(&self) -> Vec<((i32, i32), f64)> {
        ...
        for i in 0..node_list.len() {
            for j in (i + 1)..node_list.len() {
                let u = node_list[i];
                let v = node_list[j];
                let score = self.jaccard_similarity(u, v);
                if score > 0.0 {
