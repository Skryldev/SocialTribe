...
impl AlgorithmRunner for Leiden {
    fn aliases(&self) -> &'static [&'static str] {
        &["leiden", "leiden-algorithm", "leiden-community"]
    }
    ...
    fn run(&self, graph: &[Vec<Edge>], sample: bool) -> RunOutput {
        ...
        while improved && iteration < max_iterations {
            ...
            for node in 0..n {
                ...
                if neighbor_comms.is_empty() || 
                   (neighbor_comms.len() == 1 && neighbor_comms.contains_key(&current_comm)) {
                    continue;
                }
                ...
                let sum_in = self.get_community_internal_edges(
                    node, current_comm, &community, &adjacency, &mut operations
                );
                ...
                for (&comm, &edge_weight) in &neighbor_comms {
                    ...
                    let sum_in_comm = self.get_community_internal_edges(
                        node, comm, &community, &adjacency, &mut operations
                    );
            ...
            if improved {
                let refined_communities = self.refine_communities(
                    &adjacency, &community, &community_weights, total_edges, &mut operations
                );
                community = refined_communities;
            }
        ...
        let modularity = self.calculate_modularity(&community, &adjacency, total_edges, &mut operations);

        let mut community_list: Vec<(usize, usize)> = community_sizes
            .iter()
            .map(|(&k, &v)| (k, v))
            .collect();
        ...
        if sample {
            log_debug_fields!("leiden", "Leiden sampling completed", fields!(
                "vertices" => n as i64,
...
impl Leiden {
    ...
    fn refine_communities(
        ...
        let _n = adjacency.len();
        let mut refined = community.to_vec();
        let mut refined_weights: HashMap<usize, f64> = community_weights.clone();
        ...
        for (comm, nodes) in comm_nodes.iter() {
            if nodes.len() < 3 {
                continue;
            }

            let mut sub_communities: Vec<Vec<usize>> = Vec::new();
            ...
            for i in 0..nodes.len() {
                *operations += 1;
                ...
                while let Some(node) = queue.pop_front() {
                    *operations += 1;
                    ...
                    for &neighbor in &adjacency[node] {
                        *operations += 1;
                        if community[neighbor] == *comm {
                            if let Some(pos) = nodes.iter().position(|&x| x == neighbor) {
                                if !visited[pos] {
                                    visited[pos] = true;
                ...
                if sub_comm.len() > 1 {
                    sub_communities.push(sub_comm);
                }
            }

            if sub_communities.len() > 1 {
                for (idx, sub_comm) in sub_communities.iter().enumerate() {
                    let new_comm = *comm + idx + 1;
                    for &node in sub_comm {
                        refined[node] = new_comm;
                        *refined_weights.entry(new_comm).or_insert(0.0) += 
                            community_weights.get(&node).copied().unwrap_or(0.0);
                        *operations += 1;
                    }
                }
            }
    ...
    pub fn get_communities(&self, graph: &[Vec<Edge>]) -> Vec<usize> {
        ...
        while improved && iteration < max_iterations {
            ...
            for node in 0..n {
                ...
                let k_i = degrees[node];
                let sum_in = self.get_community_internal_edges(
                    node, current_comm, &community, &adjacency, &mut ops
                );
                let sum_tot = community_weights.get(&current_comm).copied().unwrap_or(0.0);
                ...
                for (&comm, &edge_weight) in &neighbor_comms {
                    ...
                    let sum_in_comm = self.get_community_internal_edges(
                        node, comm, &community, &adjacency, &mut ops
                    );
                    let sum_tot_comm = community_weights.get(&comm).copied().unwrap_or(0.0);
            ...
            if improved {
                let refined = self.refine_communities(
                    &adjacency, &community, &community_weights, total_edges, &mut ops
                );
                community = refined;
            }
        }
